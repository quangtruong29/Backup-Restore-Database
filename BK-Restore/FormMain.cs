...
namespace BK_Restore
{
	public partial class FormMain : XtraForm
	{
		public FormMain()
		{
			...
			DateTime updated = x.AddMinutes(-1);
			dtpDateRestore.DateTime = updated;
			timeRestore.Time = updated;
			//btnDelete.Click += new EventHandler(btnDeleteBackup_Click);
		}
		...
		private void btnBackup_ItemClick(object sender, ItemClickEventArgs e)
		{
			...
			if (ckbIsClearOldBackup.Checked)
			{
				...
				{
					queryString += $" , description = N'{dienGiai}'";
				}
			}
            else
            {
				...
				if (dienGiai != string.Empty)
				{
					queryString += $" WITH description = N'{dienGiai}'";
				}
			}
			...
			if (ckbIsRestoreTime.Checked)
			{
				if (IsValidTimeRestore())
				{
					string strStopAt = dtpDateRestore.DateTime.ToString("dd/MM/yyyy") + " "
												+ timeRestore.Time.ToString("h:mm:ss tt");
		...
		private bool IsValidTimeRestore()
		{
			...
			string strThoiDiemStopAt = dtpDateRestore.DateTime.ToString("dd/MM/yyyy") + " "
				+ timeRestore.Time.ToString("h:mm:ss tt");
		...
		private void ckbIsRestoreDaily_CheckedChanged(object sender, ItemClickEventArgs e)
		{
			...
			if (ckb.Checked)
			{
				...
				gvBackup.FocusedRowHandle = 0;
				
				gvBackup.SelectRow(0);
				
				//gvBackup.OptionsBehavior.EditorShowMode ;
				//gvBackup.SetFocusedRowModified = 0;
				//gcBackup.
				//gvBackup.OptionsSelection.EnableAppearanceFocusedRow = false;
				//gvBackup.OptionsSelection.EnableAppearanceFocusedRow = false;
				//gvBackup.FocusRectStyle = DevExpress.XtraGrid.Views.Grid.DrawFocusRectStyle.None;
				//gvBackup.Editable=0;
				// Hide the row indicator.
				//gvBackup.OptionsView.ShowIndicator = false;
				//gvBackup.OptionsNavigation.
				//gvBackup.OptionsSelection.EnableAppearanceFocusedCell = false;
				//gvBackup.OptionsNavigation.
				//gvBackup.OptionsSelection.MultiSelectMode
				//gcBackup.Select
			}
			else
			{
				...
				gvBackup.FocusedRowHandle = 0;
				gvBackup.SelectRow(0);
				gvBackup.OptionsSelection.EnableAppearanceFocusedRow = true;
				gvBackup.OptionsSelection.EnableAppearanceFocusedCell = true;
				//gvBackup.OptionsSelection.EnableAppearanceFocusedRow = false;
			}
		}
        ...
        private void gvBackup_FocusedRowChanged(object sender, DevExpress.XtraGrid.Views.Base.FocusedRowChangedEventArgs e)
        {
        ...
        {
			btnDeleteBackup.PerformClick();
		}

        private void bdsDevices_CurrentChanged(object sender, EventArgs e)
        {
